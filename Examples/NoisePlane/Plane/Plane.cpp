...
void Plane::Generate(const Perlin2D & noise)
{
	...
	for (Undex2D u = loop.Min(); loop.Check(u).All(true); loop.Next(u))
	{
		...
		val += noise.Calculate(p / 16.0f) * 16.0f;
		val += noise.Calculate(p / 8.0f) * 8.0f;
		val += noise.Calculate(p / 4.0f) * 4.0f;
		val += noise.Calculate(p / 2.0f) * 2.0f;
		val += noise.Calculate(p / 1.0f) * 1.0f;
