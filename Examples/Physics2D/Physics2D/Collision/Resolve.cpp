...
{
	VectorF2	ContactAbsoluteVelocity;
	VectorF2	ContactRelativePosition;

	float	LinearInertiaInverse;
	float	AngularInertiaInverse;

	float	MomentOfInertiaAxis;
	VectorF2	MomentOfInertiaCross;

	ResolveObjectData(Physics2D::Object & obj, VectorF2 pos, VectorF2 normal)
	{
		...
		MomentOfInertiaAxis = VectorF2::cross(ContactRelativePosition, normal);
		MomentOfInertiaCross = VectorF2::cross(AngularInertiaInverse * MomentOfInertiaAxis, ContactRelativePosition);
	}

	void ImpulseChange(float ImpulseScalar, VectorF2 normal, VectorF2 & change_pos, Angle & change_rot)
	{
		change_pos = (ImpulseScalar * LinearInertiaInverse) * normal;
		change_rot = Angle(Angle::Radians((ImpulseScalar * AngularInertiaInverse) * MomentOfInertiaAxis));
	}
};
...
)
{
	...
	VectorF2 Velocity = obj_data_1.ContactAbsoluteVelocity - obj_data_0.ContactAbsoluteVelocity;
	float MassInverseSum = obj_data_0.LinearInertiaInverse + obj_data_1.LinearInertiaInverse;
	VectorF2 MomentOfInertiaCrossSum = obj_data_0.MomentOfInertiaCross + obj_data_1.MomentOfInertiaCross;

	// could first calculate the dot produects and then sum
	float MomentOfInertiaSum = VectorF2::dot(MomentOfInertiaCrossSum, normal);

	//float e = 1.0f;
	float e = obj0.IntData.Restitution * obj1.IntData.Restitution;
	float ImpulseScalar = (VectorF2::dot(-(1.0f + e) * Velocity, normal)) / (MassInverseSum + MomentOfInertiaSum);
