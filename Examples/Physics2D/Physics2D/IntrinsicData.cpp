...
void Physics2D::IntrinsicData::Calculate_Area(const PolyGon & polygon)
{
	...
	for (unsigned int i = 0; i < n; i++)
	{
		...
		float area = VectorF2::cross(p0, p1);
		Area += area;
	}
	Area *= (1.0 / 2.0f);
	Mass = Area * Density;
...
void Physics2D::IntrinsicData::Calculate_MomentOfInertia(const PolyGon & polygon)
{
	...
	*/
	...
	for (unsigned int i = 0; i < n; i++)
	{
		...
		moment_of_inertia_of_triangle += VectorF2::dot(p0, p0);
		moment_of_inertia_of_triangle += VectorF2::dot(p0, p1);
		moment_of_inertia_of_triangle += VectorF2::dot(p1, p1);
