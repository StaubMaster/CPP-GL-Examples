...
{
	//	Gravity
	//	Boxes tend to stick together
	//	Gravity just makes this very bad
	/*{
		float	GravityFactor = 1.0f;
		Point2D	GravityDirection = view.Trans.Rot.rotateBack(Point2D(0, -1));
		for (unsigned int i = 0; i < Boxes.Count(); i++)
		{
			if (!Boxes[i].IsStatic)
			{
				(*Boxes[i].Instance).Vel = (*Boxes[i].Instance).Vel + (GravityDirection * (GravityFactor * timeDelta));
			}
		}
	}*/
